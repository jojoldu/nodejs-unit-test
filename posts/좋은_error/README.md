...
예외를 과도하게 사용하면 CodeSmell일 수 있다.  
반드시 유익한 것은 아니라는 것이다.  
마찬가지로 이러한 패턴을 잘못 사용하면 코드 베이스에 많은 해를 끼쳐 코드가 깨지기 쉬워지거나 오류의 원인을 모호하게 만들 수 있다.

예외를 피할 수 없다는 것은 삶의 사실이지만, 우리는 종종 예외를 신속하고 단호하게 처리해야 한다고 배운다.  
곧 알게 되겠지만 반드시 그렇지만은 않다.  
